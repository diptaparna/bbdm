...
bool cms_pp_bbdm::Initialize(const MA5::Configuration& cfg,
	...
	// Declaring signal region cuts
	string SR_1_2[]= {"SR1","SR2"};
	Manager()->AddCut("nJets<=2","SR1");
	Manager()->AddCut("pt1stJet>50",SR_1_2);
	Manager()->AddCut("pt2ndJet>30","SR1");
	Manager()->AddCut("pt2ndJet>50","SR2");
	Manager()->AddCut("pt3rdJet>30","SR2");
	Manager()->AddCut("nBtag=1","SR1");
	Manager()->AddCut("nBtag=2","SR2");
...
bool cms_pp_bbdm::Execute(SampleFormat& sample, const EventFormat& event) {
	...
	// Cut: MET > 200
	Manager()->ApplyCut(MET > 200, "MET>200");
	if (MET <= 200)
		return false;
	// Cut: No of Jets <= 3
	Manager()->ApplyCut(noOfJets <= 3, "nJets<=3");
	if (noOfJets > 3)
		return false;
	// Cut: At least 1 jet
	if (noOfJets < 1)
	...
	Manager()->ApplyCut(SignalJets[0]->pt() > 50, "pt1stJet>50");
	...
	bool SR1 = true;
	Manager()->ApplyCut(noOfJets <= 2, "nJets<=2");
	if (noOfJets == 3)
		SR1 = false;
	if (noOfJets == 2) {
		Manager()->ApplyCut(SignalJets[1]->pt() > 30, "pt2ndJet>30");
		if (SignalJets[1]->pt() <= 30
				|| SignalJets[1]->dphi_0_pi(pTmiss) <= 0.5)
			SR1 = false;
		Manager()->ApplyCut(SignalJets[0]->btag() && SignalJets[1]->btag(),
				"nBtag=1");
	...
	if (noOfJets < 2)
		SR2 = false;
	if (noOfJets == 2) {
		Manager()->ApplyCut(SignalJets[1]->pt() > 50, "pt2ndJet>50");
	...
	if (noOfJets == 3) {
		Manager()->ApplyCut(SignalJets[2]->pt() > 30, "pt3rdJet>30");
		...
		for (size_t i = 0; i < 3; i++)
			if (SignalJets[i]->btag())
				noBtaggedJets++;
		Manager()->ApplyCut(noBtaggedJets == 2, "nBtag=2");
		if (noBtaggedJets != 2)
			SR2 = false;
	}

	// Cut: No lepton or photon
	Manager()->ApplyCut(noElectron && noMuon && noTau && noPhoton, "nolepton");
